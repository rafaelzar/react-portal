...
const StarResolver: React.FC<IProps> = ({ rating = 5 }) => {
  ...
  for (let i = 0; i < rating; i += 1) {
    stars.push(<img src={star} key={`star_${i}`} alt='rating star' className='star' />);
  }
  if (isThereAHalfStar)
    stars[stars.length - 1] = <img src={halfStar} key='hlaf' alt='rating star' className='star' />;
  if (stars.length !== maxRating) {
    const missingStarsNumber = maxRating - stars.length;
    for (let i = 0; i < missingStarsNumber; i += 1) {
      stars.push(<img src={blankStar} key={`blank_${i}`} alt='rating star' className='star' />);
    }
  }
